...
- For converted holds, write `critical` on the `slide` object and on both `start`/`end` connections. The start connection uses `judgeType: "normal"`; the end connection uses `judgeType: "normal"` for release tails and `judgeType: "trace"` for trace tails.
